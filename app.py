...
if page == "Image → SVG → 3D":
    ...
    st.write("SVGs now use **direct vector geometry** instead of being rasterised and retraced. PNG/JPG files still use image tracing controls.")
    left, right = st.columns([0.40, 0.60], gap="large")
    with left:
        upload = st.file_uploader("PNG, JPG or SVG", type=["png", "jpg", "jpeg", "svg"])
        is_svg = bool(upload and upload.name.lower().endswith(".svg"))

        if is_svg:
            st.success("Direct SVG Vector Mode — original paths are used for CAD.")
            curve_tol = st.select_slider(
                "Curve accuracy",
                options=[1.00, 0.50, 0.25, 0.15],
                value=0.25,
                format_func=lambda x: {1.00:"Fast",0.50:"Standard",0.25:"High",0.15:"Ultra"}[x],
                help="Smaller tolerance keeps more points on curves. High is a good default for printable SVG artwork."
            )
        ...
        if upload and st.button("Import artwork", type="primary", use_container_width=True):
            try:
                raw = upload.getvalue()
                if is_svg:
                    with st.spinner("Reading original SVG vector paths…"):
                        polys, meta = svg_to_polygons(raw, curve_tolerance=curve_tol, max_parts=max_parts)
                        clean_svg = polygons_to_svg(polys)
                        preview = rasterize_svg(raw, 1400)
                    st.session_state.trace_contours = None
                    st.session_state.trace_size = None
                    st.session_state.trace_polygons = polys
                    st.session_state.trace_svg = clean_svg
                    st.session_state.trace_binary = None
        ...
        if st.session_state.trace_svg:
            if st.session_state.get("trace_original_svg"):
                st.download_button("Download original SVG", st.session_state.trace_original_svg, "original_design.svg", "image/svg+xml", use_container_width=True)
                st.download_button("Download CAD-clean SVG", st.session_state.trace_svg.encode("utf-8"), "cad_clean_design.svg", "image/svg+xml", use_container_width=True)
            else:
            ...
            if not exact_h:
                st.caption("Aspect ratio is locked to the imported artwork.")
            if mode == "Clay Cutter":
                c1, c2 = st.columns(2)
            ...
            if st.button("Generate 3D from vectors", type="primary", use_container_width=True):
                try:
                    polys = st.session_state.trace_polygons or []
                    if not polys:
                        raise ValueError("No vector regions are available.")
                    t0 = time.perf_counter()
                    if mode == "Clay Cutter":
                    ...
                    set_current(shape, "Vector_3D", parts, {"source": "image_trace"}, time.perf_counter() - t0)
                    st.success("3D model generated from vector geometry.")
                except Exception as e:
                    st.error(f"3D generation failed: {e}")

    with right:
        if st.session_state.get("trace_original_preview"):
        ...
        if st.session_state.get("trace_svg"):
            ...
            if meta.get("source") == "direct_svg_vector":
                ...
                c2.metric("SVG width units", f"{meta.get('width_units',0):.2f}")
                c3.metric("SVG height units", f"{meta.get('height_units',0):.2f}")
                st.caption("For SVG uploads, the CAD preview should match the original outline. No threshold or PNG tracing is involved.")
        elif st.session_state.get("trace_binary"):
